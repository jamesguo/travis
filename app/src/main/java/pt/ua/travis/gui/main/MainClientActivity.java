...
public class MainClientActivity extends MainActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(taxiList==null) {
            taxiList = CloudBackendManager.select().taxis().execute();
    ...
    private void configureSearchBar(Menu menu){
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String query) {
                ...
                filteredTaxiList = Lists.newArrayList();
    ...
    protected void fillDrawerNavigation(List<DrawerView> drawerViews) {
        ...
        Client loggedInClient = CloudBackendManager.select().clients().loggedInThisDevice();

        int numOfFavorites = loggedInClient.numberOfFavorites();
        int numOfRides = CloudBackendManager.select().rides().with(loggedInClient).uncompleted().execute().size();
...
//        drawerViews.add(new DrawerView(5, R.string.menu_settings, R.drawable.ic_action_settings));
    ...
    public void goToScheduledRidesList(View view){
        ...
        getSupportFragmentManager()
                .beginTransaction()
                .replace(R.id.content_frame, RideListFragment.newInstance(RideItem.SHOW_TAXI, rideList))
                .addToBackStack(null)
                .commit();
    }
    ...
    public void showFavorites(View view) {
        ...
        filteredTaxiList = CloudBackendManager.select().taxis().favoritedBy(loggedInClient);
    ...
    public void onLaterButtonClicked(View view) {
        ...
        Address currentAddress = Utils.latlngToAddress(this, pos.latitude, pos.longitude);
        String currentAddressString = Utils.addressToString(currentAddress);

        rideBuilder.resetToHereAndNow();
        pager.setCurrentItem(1, true);
        addressTextView = (TextView) findViewById(R.id.origin_address);
        addressTextView.setText(currentAddressString);

        TimePicker timePicker = (TimePicker) findViewById(R.id.timePicker);
        timePicker.setIs24HourView(true);

        Calendar now = Utils.newTime().toNow();
    ...
    public void onDoneButtonClicked(View view){
        TimePicker timePicker = (TimePicker) findViewById(R.id.timePicker);
        rideBuilder.setScheduledTime(Utils.newTime()
                .withHourOfDay(timePicker.getCurrentHour())
                .andWithMinute(timePicker.getCurrentMinute()));
        Ride newRide = rideBuilder.build();
