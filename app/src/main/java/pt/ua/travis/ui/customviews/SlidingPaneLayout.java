...
public class SlidingPaneLayout extends FrameLayout {
    ...
    /**
     * The withUser of the panel when closed
     */
     ...
     * <ol>
     *     <li>{@link #setStickTo(int)} withUser param {@link #STICK_TO_AUTO}</li>
     * 	<li>Use no shadow drawable. (i.e. withUser width of 0)</li>
    ...
    public SlidingPaneLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        // Style
        final TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SlidingPaneLayout);

        // Set the side of the screen
        setStickTo(ta.getInt(R.styleable.SlidingPaneLayout_stickTo, STICK_TO_AUTO));

        // Sets the shadow drawable
        int shadowRes = ta.getResourceId(R.styleable.SlidingPaneLayout_shadowDrawable, -1);
        if (shadowRes != -1) {
            setShadowDrawable(shadowRes);
        }

        // Sets the shadow width
        setShadowWidth((int) ta.getDimension(R.styleable.SlidingPaneLayout_shadowWidth, 0));

        // Sets the ability to close the layer by tapping in any empty space
        closeOnTapEnabled = ta.getBoolean(R.styleable.SlidingPaneLayout_closeOnTapEnabled, true);
        // Sets the ability to open the layout by tapping on any of the exposed closed layer
        openOnTapEnabled = ta.getBoolean(R.styleable.SlidingPaneLayout_openOnTapEnabled, true);

        // How much of the view sticks out when closed
        setOffsetWidth(ta.getDimensionPixelOffset(R.styleable.SlidingPaneLayout_offsetWidth, 0));
     ...
     * the panel was opened withUser smooth animation this method is not guaranteed to return
    ...
    /**
     * Return the current withUser of the shadow.
     * 
     ...
     *            the number of pixels to scroll by on the Y axis
     * @param velocity
     *            the velocity associated withUser a fling, if applicable. (0 otherwise)
     */
    ...
    private void completeScroll() {
        ...
        if (needPopulate) {
            // Done withUser scroll, no longer want to cache view drawing.
            setDrawingCacheEnabled(false);
    ...
    /**
     * Sets the default location where the SlidingPaneLayout will appear
    ...
    /**
     * If parameter is set to <code>true</code>, whenever the <code>SlidingPaneLayout</code> is tapped and
     * the SlidingPaneLayout is opened, it will attempt to close.
     * If parameter is set to <code>false</code>, then tapping the <code>SlidingPaneLayout</code> will
     * do nothing
     * 
     ...
     * to true it will attempt to open the <code>SlidingPaneLayout</code>. If parameter is false,
    ...
    public void computeScroll() {
        if (!mScroller.isFinished()) {
            if (mScroller.computeScrollOffset()) {
                ...
                // We invalidate a slightly larger area later, this was only optimised for right menu previously
        ...
        // Done withUser scroll, clean up state.
        completeScroll();
    }

    /**
     * Handler interface for obtaining updates on the <code>SlidingPaneLayout</code>'s state.
     * <code>OnInteractListener</code> allows for external classes to be notified when the <code>SlidingPaneLayout</code>
     * receives input to be opened or closed.
     */ 
    public interface OnInteractListener {

        /**
    	 * This method is called when an attempt is made to open the current <code>SlidingPaneLayout</code>. Note
    	 * that because of animation, the <code>SlidingPaneLayout</code> may not be visible yet.
    	 */
        public void onOpen();
        
        /**
        * This method is called when an attempt is made to close the current <code>SlidingPaneLayout</code>. Note
    	* that because of animation, the <code>SlidingPaneLayout</code> may still be visible.
    	*/
        public void onClose();
        ...
        /**
         * this method is executed after <code>onClose()</code>, when the animation has finished and the  <code>SlidingPaneLayout</code> is
