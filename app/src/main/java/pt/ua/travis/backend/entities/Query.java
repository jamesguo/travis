...
//                            int minuteCompare = minute1.compareTo(minute2);
        ...
        protected void fetchLater(final ParseObject po, final Callback<Ride> instantiateCallback) {
            ...
            PersistenceManager.query().clients().withId(cId).later(new Callback<List<Client>>() {
                @Override
                public void onResult(List<Client> result) {
                    final Client c = result.get(0);

                    PersistenceManager.query().taxis().withId(tId).later(new Callback<List<Taxi>>() {
                        @Override
                        public void onResult(List<Taxi> result) {
                            final Taxi t = result.get(0);

                            instantiateCallback.onResult(new Ride(po, c, t));
                        }
                    });
                }
            });
